...
public class PassengerEditor extends JFrame {
	...
	public PassengerEditor(String email) {
		...
		txtEmailaddress.setBounds(10, 11, 225, 20);
		...
		txtFirstname.setBounds(10, 42, 191, 20);
		...
		txtLastname.setBounds(10, 73, 225, 20);
		...
		btnSave.setBounds(10, 104, 225, 23);
		...
		btnRefresh.setBounds(10, 138, 225, 23);
		contentPane.add(btnRefresh);
		
		/*
		 * btnDelete = new JButton("Delete"); btnDelete.setEnabled(false);
		 * btnDelete.setBounds(10, 172, 225, 23); contentPane.add(btnDelete); try
		 * (Connection con = DriverManager.getConnection(connectionUrl); Statement stmt
		 * = con.createStatement();) { String SQL =
		 * ("SELECT * FROM PASSENGERS WHERE EmailAddress = '"+emailAddress+"';");
		 * ResultSet rs = stmt.executeQuery(SQL); rs.next();
		 * txtFirstname.setText(rs.getString("FName"));
		 * txtLastname.setText(rs.getString("Lname"));
		 * txtEmailaddress.setText(emailAddress);
		 * txtPid.setText(rs.getString("PassengerID")); } catch (SQLException e) {
		 * e.printStackTrace(); }
		 */
